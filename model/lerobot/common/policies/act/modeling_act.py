...
from model.lerobot.common.policies.normalize import Normalize, Unnormalize
...
class ACTPolicy(
    ...
    def forward(self, batch: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        batch["action"]= self.unnormalize_outputs({"action": batch["action"]})["action"]
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.use_vae:
            ...
            # dimension.
            num_input_token_encoder = 1 + config.n_action_steps
            if self.use_robot_state:
    ...
    def forward(self, batch: Dict[str, Tensor]) -> Tuple[Tensor, Union[Tuple[Tensor, Tensor], Tuple[None, None]]]:
        ...
        if self.config.use_vae and "action" in batch:
            ...
            cls_joint_is_pad = torch.full(
                ...
                False,
                device=batch["observation.image.color"].device,
            )
