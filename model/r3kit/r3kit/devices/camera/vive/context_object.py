"""
Modified from https://github.com/cmbruns/pyopenxr/blob/main/src/xr/context_object.py
and https://github.com/cmbruns/pyopenxr_examples/blob/main/xr_examples/vive_tracker.py
"""
...
class ContextObject(object):
    ...
    def _enter(self):
        ...
        # NOTE: do not use the swapchains
...
if __name__ == '__main__':
        ...
        if context.session_state == xr.SessionState.FOCUSED:
            ...
            for index, space in enumerate(tracker_action_spaces):
                ...
                if space_location.location_flags & xr.SPACE_LOCATION_POSITION_VALID_BIT:
                    print(f"{role_strings[index]}: " + \
                          f"{[space_location.pose.position.x, space_location.pose.position.y, space_location.pose.position.z]} " + \
                          f"{[space_location.pose.orientation.x, space_location.pose.orientation.y, space_location.pose.orientation.z, space_location.pose.orientation.w]}")
    ...
    if not session_was_focused:
        print("This OpenXR session never entered the FOCUSED state. Did you modify the headless configuration?")
