...
class RISEPolicyWrapper:
    ...
    def preprocess_image(self, image, depth, cam_intrinsics):
        """
        根据RGB图像对齐裁剪深度图，并同步修正相机内参
        
        Args:
            image: RGB图像 (480, 640, 3) uint8
            depth: 深度图 (480, 640) uint16 毫米单位
            cam_intrinsics: 原始相机内参 (3, 4)
        
        Returns:
            cropped_rgb: 裁剪后的RGB图像
            cropped_depth: 裁剪后的深度图
            adjusted_intrinsics: 修正后的相机内参（光心已根据裁剪偏移调整）
        """
        # 确保输入是numpy数组
        ...
        # 裁剪参数（与训练数据准备保持一致：vis_cloud.py）
        start_w = 200  # 修正为与训练一致
        end_w = 560
        start_h = 0
        end_h = 360
        ...
        # 修正内参：裁剪后光心位置需要减去裁剪偏移
        fx, fy = cam_intrinsics[0, 0], cam_intrinsics[1, 1]
        cx, cy = cam_intrinsics[0, 2], cam_intrinsics[1, 2]
        
        adj_cx = cx - start_w  # 光心X坐标减去横向裁剪起点
        adj_cy = cy - start_h  # 光心Y坐标减去纵向裁剪起点
        
        adjusted_intrinsics = cam_intrinsics.copy()
        adjusted_intrinsics[0, 2] = adj_cx
        adjusted_intrinsics[1, 2] = adj_cy
        
        return cropped_rgb, cropped_depth, adjusted_intrinsics
    ...
    def predict_single_action(self, images, current_state, cam_intrinsics):
        ...
        # 裁剪图像并获取修正后的内参（关键修复！）
        color_img, depth_img, adjusted_intrinsics = self.preprocess_image(
            color_img, depth_img, cam_intrinsics
        )
        
        # 创建点云输入（使用修正后的内参）
        coords_batch, feats_batch, cloud = self.create_input(
            color_img, depth_img, adjusted_intrinsics
        )
